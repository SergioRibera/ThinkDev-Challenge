...
describe(" /users", function () {
  ...
  it("when send /users should get all users ", async () => {
    const response = await request(app).get("/users");
    let resUsers = response.body;

    expect(response.body.length).toEqual(6);

    expect(JSON.stringify(resUsers)).toEqual(JSON.stringify(users));
  });

  it("when send /users?other=value should get all users and ignore query ", async () => {
    const response = await request(app).get("/users?other=value");
    let resUsers = response.body;
